...
for i in range(len(headers)):
    ...
    idsNames = {}
    for j in range(len(dat[game_id_index])):  
        game_id = dat[game_id_index][j]  
        if game_id not in idsNames:
            idsNames[game_id] = len(idsNames)  # Assign a new ID to each game
    
    ids = {}
    idsToNames = []
    
    for j in range(len(dat[i])):  # Loop over the current column data
        value = dat[i][j]
        
        # Ensure each unique value gets a unique ID
        if value not in ids:
            ids[value] = {'id': len(ids)}

        # Create the idsToNames mapping (value ID, game ID)
        game_id = dat[game_id_index][j]  # Reference the game_id for this row
        idsToNames.append([ids[value]['id'], idsNames[game_id]])  # Mapping: [value_id, game_id]

    newIds = []
    for k in ids.keys():
        newIds.append([k, ids[k]['id']])  # Create mapping: [value, value_id]
